...
const ChatList = memo<ChatListProps>(
  ({
    ...
    // Agent-share visitors are logged in but NOT the owner: the owner-scoped
    // config API would resolve to null and `markAgentNotFound` would wipe the
    // share-seeded agentMap entry, so the fetch is skipped entirely — the
    // share layout already seeds the meta from `getSharedAgent`.
