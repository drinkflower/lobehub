...
const PROVIDER = 'minimax-code';

interface MinimaxCodeAcpPayload {
  [key: string]: unknown;
  content?: unknown;
  input?: unknown;
  kind?: unknown;
...
interface MinimaxCodeAcpToolContent {
  content?: unknown;
  newText?: unknown;
  path?: unknown;
  terminalId?: unknown;
...
interface MinimaxCodeAcpToolResultState {
  content?: unknown;
...
const toolContent = (content: unknown, fallback: unknown): string => {
  ...
  const result = content
    .map((value) => {
      const block = value as MinimaxCodeAcpToolContent | null;
...
export class MinimaxCodeAcpAdapter implements AgentEventAdapter {
  sessionId?: string;
  ...
  private toolResultStateById = new Map<string, MinimaxCodeAcpToolResultState>();
  ...
  adapt(value: unknown): HeterogeneousAgentEvent[] {
    ...
    const raw = value as MinimaxCodeAcpPayload;
    if (raw.type === 'session_configured') {
      if (typeof raw.model === 'string') this.model = raw.model;
      return [];
    }
    if (raw.type === 'minimax_code_session') {
      if (typeof raw.sessionId === 'string') this.sessionId = raw.sessionId;
      if (typeof raw.model === 'string') this.model = raw.model;
      return [];
    }
    if (raw.type === 'minimax_code_prompt_completed') return this.complete(raw.stopReason);
    if (raw.type === 'minimax_code_error') {
      return this.fail(stringify(raw.message) || 'MiniMax Code ACP failed');
    }
  ...
  private updateTool(raw: MinimaxCodeAcpPayload): HeterogeneousAgentEvent[] {
  ...
  private startTool(raw: MinimaxCodeAcpPayload): HeterogeneousAgentEvent[] {
    const id = raw.toolCallId;
    if (typeof id !== 'string' || this.completedTools.has(id)) return [];
  ...
  private mergeToolResultState(
    toolCallId: string,
    raw: MinimaxCodeAcpPayload,
  ): MinimaxCodeAcpToolResultState {
